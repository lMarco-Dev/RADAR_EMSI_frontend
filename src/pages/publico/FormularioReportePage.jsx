...
import { Lock, Radar, ShieldCheck, Loader2, Search, X, Clock, CheckCircle, AlertCircle, FileText } from "lucide-react"; 
...
export default function FormularioReportePage() {
  ...
  const handleRastrear = async (e) => {
    ...
    try {
      const { data } = await axiosInstance.get(`/publico/reportes/rastrear/${folioSearch.trim()}`);
      setTrackerResult(data.data);
    } catch (error) {
      setTrackerError(error.response?.data?.message || "No se encontró ningún reporte con este folio.");
    } finally {
        ...
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm flex justify-center items-center z-50 p-4">
          <div className="bg-white rounded-3xl w-full max-w-lg max-h-[90vh] overflow-y-auto shadow-2xl p-6 md:p-8 animate-in zoom-in duration-200">
            <div className="flex justify-between items-start mb-6">
              <div>
                <h2 className="text-2xl font-black text-slate-800 flex items-center gap-2"><Search className="text-blue-600" /> Rastreador de Folio</h2>
                <p className="text-slate-500 text-sm mt-1">Consulta el estado de tu reporte de forma anónima.</p>
              </div>
              <button onClick={() => { setIsTrackerOpen(false); setTrackerResult(null); setTrackerError(""); setFolioSearch(""); }} className="p-2 bg-slate-100 text-slate-500 hover:bg-slate-200 rounded-full transition-colors"><X size={20}/></button>
            </div>
            ...
            <form onSubmit={handleRastrear} className="flex gap-2 mb-6">
              <input 
                ...
                required
              />
              <Button type="submit" disabled={trackerLoading} className="px-6 rounded-xl">
                {trackerLoading ? <Loader2 className="animate-spin" size={20} /> : "Buscar"}
              </Button>
            </form>
              ...
              <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
                ...
                <div className="grid grid-cols-2 gap-4 bg-slate-50 p-5 rounded-2xl border border-slate-100">
                  <div className="col-span-2 flex justify-between items-center border-b border-slate-200 pb-3 mb-1">
                    <p className="font-black text-lg text-slate-800">{trackerResult.folio}</p>
                    {getEstadoBadge(trackerResult.estado)}
                  </div>
                  <div>
                    <p className="text-[10px] text-slate-400 font-black uppercase tracking-widest">Tipo</p>
                    <p className="font-semibold text-slate-700 text-sm">{trackerResult.tipoComportamientoNombre}</p>
                  </div>
                  <div>
                    <p className="text-[10px] text-slate-400 font-black uppercase tracking-widest">Fecha Registrado</p>
                    <p className="font-semibold text-slate-700 text-sm">{formatearFecha(trackerResult.createdAt)}</p>
                  </div>
                </div>

                {/* Línea Temporal Pública */}
                <div>
                  <h3 className="text-md font-bold text-slate-800 mb-4 flex items-center gap-2"><Clock size={18} className="text-blue-500"/> Historial de Seguimiento</h3>
                  <div className="space-y-6 border-l-2 border-slate-100 ml-3 pl-6 relative">
                    {trackerResult.historial?.length > 0 ? (
                      trackerResult.historial.map((hist, idx) => (
                        <div key={idx} className="relative">
                          ...
                          <p className="font-bold text-slate-700 mt-0.5">Estado: {hist.estadoNuevo.replace("_", " ")}</p>
                          {hist.comentario && <p className="text-sm text-slate-600 bg-white border border-slate-100 p-3 rounded-xl mt-2 italic shadow-sm">"{hist.comentario}"</p>}
                        </div>
                      ))
                    ) : (
                      <p className="text-sm text-slate-400 italic">Tu reporte ha sido recibido y está pendiente de revisión.</p>
                    )}
                    ...
                    <div className="relative">
                      ...
                      <p className="text-xs font-bold text-slate-400 uppercase">{formatearFecha(trackerResult.createdAt)}</p>
                      <p className="font-bold text-slate-700 mt-0.5">Reporte Registrado</p>
                    </div>
                  </div>
                </div>
              </div>
